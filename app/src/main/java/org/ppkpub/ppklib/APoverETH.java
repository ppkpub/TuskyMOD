...
public class APoverETH {
  ...
  protected static String toABIHex(String input) throws Exception{
    ...
    try {
      data = input.getBytes(PPkDefine.BINARY_DATA_CHARSET);
      dataArrayList = PPkUtil.toByteArrayList(data);
    ...
    if( data_length > 0 ){
        ...
        strHexABI=strHexABI+PPkUtil.bytesToHexString(tmpLenBytes);
        
        //生成具体字符串内容HEX文本
        strHexABI=strHexABI+PPkUtil.bytesToHexString(data);
    }
  ...
  protected static String getFirstSegmentFromABIHex( String str_hex ) throws Exception{
    byte[] tmpBytes=PPkUtil.hexStringToBytes(str_hex);
    List<Byte> dataArrayList =  PPkUtil.toByteArrayList(tmpBytes);
    ...
    byte[] chunk = PPkUtil.toByteArray( new ArrayList<Byte>(dataArrayList.subList(65, 65+content_len) ) );
    
    return new String(chunk,PPkDefine.PPK_TEXT_CHARSET); 
  }
}